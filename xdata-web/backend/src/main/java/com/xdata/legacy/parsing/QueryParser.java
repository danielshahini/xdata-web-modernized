...
import com.xdata.legacy.parsing.Conjunct_ToDel;
import com.xdata.legacy.parsing.ForeignKey;
import com.xdata.legacy.parsing.FromListElement;
...
import com.xdata.legacy.parsing.Utility;
...
//					ProcessSelectClause.ProcessSelect(plainSelect, debug, this);
				...
				if(((Select) stmt) instanceof PlainSelect 
					...
					ProcessResultSetNode.processResultSetNodeJSQL((PlainSelect)((Select) stmt), debug, this,dbApparameters);
					//ProcessSelectClause.ProcessSelect((PlainSelect)((Select) stmt).getSelect(), debug, this);
				}
	 ...
	 *  Splits the Select into Left (in this case <A>), and Join ( in the above case <Natural Join B>), deal with them separately 
	 */
	...
	Select transformSubJoinForWithAs(WithItem srcWithItem, Select subJoin){
		...
		if(leftFromItem instanceof net.sf.jsqlparser.schema.Table){			
			...
			if(tarTable.getName().equalsIgnoreCase(srcWithItem.getAlias().getName())){
				...
				//Get withItem and create new ParenthesedSelect if fromItem name is equal to with item name
				Alias a;
		...
		//checks if the left item (assumed to be a  Table or Select itself) is a SubJoin, if then recursive call
		else if(leftFromItem instanceof Select){
			transformSubJoinForWithAs(srcWithItem,(Select)leftFromItem);
		}
		// if fromitem is a ParenthesedSelect , then call the corresponding method that handles it
		else if(leftFromItem instanceof ParenthesedSelect){
			logger.info("processing ParenthesedSelect");
		...
		//checks if the right item (assumed to be a  Table or Select itself) is a Table, then if its name is the same as the name of the 
		...
		//checks if the right item (assumed to be a  Table or Select itself) is a SubJoin, if then recursive call
	...
	PlainSelect transformPlainSelectForWithAs(WithItem srcWithItem, PlainSelect tarSelectClause){
		// Starts by dealing with from items, a from item can be a Table name, a ParenthesedSelect statement, or a Select statement
	...
	/** @author mathew  
	 *  Extract the select statement from the ParenthesedSelect. It can be either a PlainSelect 
	 ...
	 *  of the name of the input with item, if any occurence is found its corresponding definition is substituted as a ParenthesedSelect
	...
	private void transformJoinsForWithAs(WithItem srcWithItem, List<Join> joinList){
		for(int k=0; k < joinList.size(); k++){
			...
			//if join item is a ParenthesedSelect, then call the corresponding method that handles it
			else if(tarJoinFromItem instanceof ParenthesedSelect){
				logger.info("processing ParenthesedSelect in join");
			...
			//if join item is a subjoin, then call the corresponding method that handles it
				...
				logger.info("processing Select in join");
	...
	private void transformWhereClauseForWithAs(WithItem srcWithItem, Expression whereClause) {
		//if whereClause is a ParenthesedSelect, then call the corresponding method that handles it
		...
		//if whereClause is a All comparision Expression, then recurisively call using its ParenthesedSelect as argument
			...
			transformPlainSelectForWithAs(srcWithItem, (PlainSelect)(((ParenthesedSelect)ace).getSelect()));
		}
		//if whereClause is a Any comparision Expression, then recurisively call using its ParenthesedSelect as argument
			...
			Expression exp = (Expression) ace;
			ParenthesedSelect ss = (ParenthesedSelect) exp;
	...
	private WithItem normalizeWithItem(WithItem withItem) {
		...
		logger.info(" normalized with item "+withItem.getAlias().getName()+" withItemBody: "+withItem.getSelect());
		return withItem;
	}
