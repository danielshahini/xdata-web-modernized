...
//				cvc.getTableMap().getTable(column.getTableName()).
				...
				if(columnValue.size()>0){
					for(int j=0; j<columnValue.size(); j++){
						...
						String strColValue=com.xdata.legacy.util.Utilities.covertDecimalToFraction(colValue+"");
						//Commented out by Biplab
				...
				//Adding support for NULLs
				//Consider the range -99996 to -99999 as NULL integers
				String maxStr=com.xdata.legacy.util.Utilities.covertDecimalToFraction(max+"");
				String minStr=com.xdata.legacy.util.Utilities.covertDecimalToFraction(min+"");
			...
			if((colMap.containsKey(columnName) && colMap.get(columnName).getLeft().getCheckValues().size() < column.getCheckValues().size() ) || !colMap.containsKey(columnName) || column.limitDefined()){//prevent repetition of data types in cvc file
				...
				colMap.put(columnName, new ImmutablePair<Column,String>(column, cvc_datatype) );
