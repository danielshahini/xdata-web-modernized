...
public class WhereClauseVectorJSQL {
	...
	public static void getAggregationDataStructures(PlainSelect rsNode,FromListElement queryAliases, Map<String,Table> fromTables,boolean fromSubquery,boolean whereSubquery, QueryParser qParser, AppTest_Parameters dbApparameters) throws Exception {
		...
		/*	try{
			...
			for(Node each: tempGroupBy){
				com.xdata.legacy.parsing.Column colName = each.getColumn();
				...
				if(originalTable.hasForeignKey()){
					for (String fKeyName : originalTable.getForeignKeys().keySet()) {
						...
						Vector<com.xdata.legacy.parsing.Column> fKeyColumns = fKey.getFKeyColumns();
						Vector<Node> nd = null;
						for(com.xdata.legacy.parsing.Column fk : fKeyColumns ){
							if(!(each.getColumn().getColumnName().equalsIgnoreCase(fk.getColumnName()))
									&& !(each.getColumn().equals(fKey.getReferenceKeyColumns()))){
								for(com.xdata.legacy.parsing.Column c : fKey.getReferenceKeyColumns()){
				...
				if(rc.getExpression() instanceof Function){					
					...
					tempAggFunc.add(n.getAgg()); 
						...
						if(aggFunc.getAggExp() == null){
							if(i>0 && rcList.get(i-1) instanceof SelectItem){
								SelectItem rcPrev = (SelectItem)rcList.get(i-1);
	...
	try{
		if (clause == null) {
			return null;
		} else if (clause instanceof ParenthesedExpressionList){
		...
		//**********************************************************************************/
			...
			/*if(sqn.getLeftExpression() instanceof ExtractExpression) {
				...
				Node n = new Node();
				return n;
			}else if(sqn.getLeftExpression() instanceof ExtractExpression){
				Node n = new Node();
				return n;
			}
			else if(sqn.getRightExpression() instanceof ExtractExpression){
				Node n = new Node();
				return n;
			}*/					
			...
			if (sqn. getLeftExpression() instanceof ParenthesedSelect){
				//Shree changed isWhereclause to true
				/*if(isWhereClause 
						&& qParser.getWhereClauseSubqueries() != null
						&& !qParser.getWhereClauseSubqueries().isEmpty()){
				OperateOnSubQueryJSQL.OperateOnSubquery((ParenthesedSelect)sqn.getLeftExpression(), thisSubQConds,new JoinTreeNode(),false,true,qParser.getWhereClauseSubqueries().get(qParser.getWhereClauseSubqueries().size()-1));
				}
				else{*/
					OperateOnSubQueryJSQL.OperateOnSubquery((ParenthesedSelect)sqn.getLeftExpression(), thisSubQConds,new JoinTreeNode(),false,true,qParser,dbApparameters);
				//} 
			}
			...
			/*	if(isWhereClause 
						&& qParser.getWhereClauseSubqueries() != null
						&& !qParser.getWhereClauseSubqueries().isEmpty()){
				OperateOnSubQueryJSQL.OperateOnSubquery((ParenthesedSelect)sqn.getRightExpression(), thisSubQConds,new JoinTreeNode(),false,true,qParser.getWhereClauseSubqueries().get(qParser.getWhereClauseSubqueries().size()-1));
				} 
				else{*/
					OperateOnSubQueryJSQL.OperateOnSubquery((ParenthesedSelect)sqn.getRightExpression(), thisSubQConds,new JoinTreeNode(),false,true,qParser,dbApparameters);
				//}
			}
			...
			if(subS != null){
			List<SelectItem<?>> rcList = ((PlainSelect)subS.getSelect()).getSelectItems();
			
			if (rcList.get(0).getExpression() instanceof AllColumns ) {
			...
			else if (rcList.get(0) instanceof SelectItem){		
		...
		//	Node newCaseNode = new Node();
			Node n = new Node();
			List<WhenClause> whenExprList = expr.getWhenClauses();
		...
		else if (clause instanceof Expression){
			Vector<Node> thisSubQConds = new Vector<Node>();
			//FIXME: mahesh changed this.But not sure
			Expression ace = (Expression)clause;

			ParenthesedSelect ss = (ParenthesedSelect) ace;
			//Shree changed isWhereclause to true
			...
			// Extract the projected column and create a colref node
			PlainSelect sbod = (PlainSelect)ss.getSelect();
			List<SelectItem<?>> siList = sbod.getSelectItems();
			Node rhs=null;
			SelectItem sei = siList.get(0);
			int i=0;
			if (sei instanceof SelectItem){

				Expression exp1 = ((SelectItem)sei).getExpression();
				...
				if(name.equalsIgnoreCase("year")){
					...
					Node nr1 = new Node();
							...
							Node nr1l1 = new Node();
								...
								if(queryType == 2) nr1l1.setQueryIndex(qParser.getWhereClauseSubqueries().size()-1);
										...
										Node nr1l2 = new Node();
											...
											Node nr1r3 = new Node();
												...
												String st="30";
												st=com.xdata.legacy.util.Utilities.covertDecimalToFraction(st);
								...
								nr1l1.setLeft(nr1l2);	
									//Right Node at level 2
										...
										String s1="12";
										s1=com.xdata.legacy.util.Utilities.covertDecimalToFraction(s1);
						...
						//Right Node at level 1
							...
							String s2="12";
							s2=com.xdata.legacy.util.Utilities.covertDecimalToFraction(s2);
...
//				if(queryType == 2) n.setQueryIndex(qParser.getWhereClauseSubqueries().size()-1);
				...
				//ProcessSelectClause.processWhereParenthesedSelect(subS,subQueryParser,qParser);
				...
				//ProcessSelectClause.processWhereParenthesedSelect(sqn,subQueryParser,qParser);
...
//				if(queryType == 2) n.setQueryIndex(qParser.getWhereClauseSubqueries().size()-1);
				...
				//ProcessSelectClause.processWhereParenthesedSelect(ss,subQueryParser,qParser);
