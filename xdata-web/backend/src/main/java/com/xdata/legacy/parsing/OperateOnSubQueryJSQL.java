...
public class OperateOnSubQueryJSQL {
	...
	public static FromListElement OperateOnSubquery(ParenthesedSelect subquery,Vector<Node> allConds, JoinTreeNode jtn,boolean fromSubquery, boolean whereSubquery, com.xdata.legacy.parsing.QueryParser qParser,AppTest_Parameters dbApparameters) throws Exception {
		try{
		String aliasName = "";
		...
		com.xdata.legacy.parsing.QueryParser fromClause=new com.xdata.legacy.parsing.QueryParser(qParser.getTableMap());
		fromClause.setQuery(qParser.getQuery());
		com.xdata.legacy.parsing.QueryParser whereClause=new com.xdata.legacy.parsing.QueryParser(qParser.getTableMap());
		...
		if((subquery.getSelect()) instanceof SetOperationList){
			isSetOp = true;
			SetOperationList setOpList = (SetOperationList)( subquery.getSelect());
		...
		if(fromSubquery && !isSetOp){
			...
			PlainSelect plainSelect = (PlainSelect)subquery.getSelect();
		...
		if(whereSubquery && !isSetOp){
			...
			if (plainSelect.getJoins()==null){
				...
				if(prev instanceof net.sf.jsqlparser.schema.Table){
					...
					FromListElement temp = 	temp = OperateOnSubQueryJSQL.OperateOnSubquery(((ParenthesedSelect)prev), allConds, jtn, fromSubquery, whereSubquery, qParser, dbApparameters);
