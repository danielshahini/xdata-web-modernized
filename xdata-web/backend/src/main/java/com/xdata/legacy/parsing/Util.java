...
import com.xdata.legacy.parsing.QueryAliasMap;
...
//	}
	...
	public static Vector<Column> getJoinColumnsJSQL(String columnName, FromItem node, QueryParser qParser)
		...
		} else if (node instanceof ParenthesedSelect) {
			ParenthesedSelect fromSubquery = (ParenthesedSelect) node;
			PlainSelect ps = (PlainSelect)fromSubquery.getSelect();
...
public static Vector<Column> getColumnsForCountstar(Vector<FromClauseElement> fle, QueryStructure qParser) {
	Vector<Column> columns = new Vector<Column>();
	for(FromClauseElement f:fle){
		String fromTableName = f.getTableName();
		if (fromTableName != null && f.getTableNameNo()!= null &&!f.getTableNameNo().isEmpty()) {
			Table t=qParser.getTableMap().getTable(fromTableName);
			columns.addAll(t.getColumns().values());
			...
			for (int i = 0; i < f.getBag().size(); i++) {
				columns = getColumnsForCountstar(f.getBag().get(i), qParser);
				// n.setType(Node.getColRefType());
				// if (n != null) {
				// 	return n;
				// }
			...
			columns=getColumnsForCountstar(f.getSubQueryStructure().fromListElements,f.getSubQueryStructure());
			// if(n!=null){
			// 	n.setType(Node.getColRefType());
			// 	return n;
			// }
...
public static Vector<Column>  getColumnsForCountstar(FromClauseElement f, QueryStructure qParser) {
	Vector<Column> columns = new Vector<Column>();
