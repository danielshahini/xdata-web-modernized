...
import com.xdata.legacy.parsing.JoinClause;
import com.xdata.legacy.parsing.JoinClauseInfo;
import com.xdata.legacy.parsing.JoinClauseNew;
		...
		int index = 0;
				...
				while(joinsIt.hasNext()){
					...
					if(join.getRightItem() instanceof Select){
						...
						frmItem = ((Select)join.getRightItem()).getPlainSelect().getJoin(0).getFromItem();
						Join aJ = ((Select)join.getRightItem()).getPlainSelect().getJoin(0);
	...
	public static FromListElement OperateOnJoinNode(Join joinNode,FromItem rt,FromItem lt, Vector<FromListElement> frmItemList,
		...
		if(lt != null){
			if (lt instanceof net.sf.jsqlparser.schema.Table) {
				...
				t.add(temp);
				leftFle.add(temp);
			} else if (lt instanceof ParenthesedSelect) {
				FromListElement temp;
				OperateOnSubQueryJSQL.OperateOnSubquery( (ParenthesedSelect)lt, allConds, leftChild, true, false,qParser, dbApparameters);
		...
		if (rt instanceof net.sf.jsqlparser.schema.Table) {
			...
			t.add(temp);
			rightFle.add(temp);
		} else if (rt instanceof ParenthesedSelect) {
			FromListElement temp;
			OperateOnSubQueryJSQL.OperateOnSubquery((ParenthesedSelect) rt, allConds, rightChild,true,false,qParser, dbApparameters);
			temp=qParser.getFromClauseSubqueries().get(qParser.getFromClauseSubqueries().size()-1).getQueryAliases();
			t.add(temp);
			//@author mathew on 28 June 2016, the following line added for enabling recursive parsing of subqueries 
		...
		if (joinNode.getUsingColumns() != null || joinNode.isNatural() || joinNode.isOuter()) {
			...
			if (lt != null && lt instanceof ParenthesedSelect ){//&& lt instanceof ParenthesedSelect ){//If sub query then we should add only the projected columns
			...
			if (rt instanceof ParenthesedSelect ) {//If sub query then we should add only the projected columns
			...
			for (int i = 0; i < columnList.size(); i++) {
				...
				if(rt instanceof Select){
					joinColumns2 = Util.getJoinColumnsJSQL(column.getColumnName().toUpperCase(), ((Select) rt).getPlainSelect().getJoin(0).getRightItem(),qParser);
					if(joinColumns2 == null || (joinColumns2 != null && joinColumns2.isEmpty())){
						joinColumns2 = Util.getJoinColumnsJSQL(column.getColumnName().toUpperCase(), ((Select) rt).getPlainSelect().getJoin(0).getFromItem(),qParser);
					}
				}else{
				...
				for (int j = 0; j < joinColumns1.size(); j++) {
					for (int k = 0; k < joinColumns2.size(); k++) {
						...
						if (lt != null && lt instanceof ParenthesedSelect){ 
						...
						if(frmItemList!= null && !frmItemList.isEmpty() && !(rt instanceof Select)){
							left = Util.getColumnFromOccurenceInJC(leftCol
						...
						Node right = new Node();
						if (rt instanceof ParenthesedSelect){ 
