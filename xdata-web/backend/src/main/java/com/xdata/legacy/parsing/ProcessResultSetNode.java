...
import com.xdata.legacy.generateConstraints.UtilsRelatedToNode;
...
import com.xdata.legacy.testDataGen.GenerateDataSet;
...
public class ProcessResultSetNode {
	...
	public static void processResultSetNodeJSQL(PlainSelect plainSelect,boolean debug,com.xdata.legacy.parsing.QueryParser qParser,AppTest_Parameters dbApparameters) throws Exception{
		try{
		...
		if (plainSelect.getJoins()==null && ! (prev instanceof Select)){
			//only one item
			if (prev instanceof net.sf.jsqlparser.schema.Table){
				...
				t.add(temp);
			} else if (prev instanceof ParenthesedSelect){
				FromListElement temp =	OperateOnSubQueryJSQL.OperateOnSubquery((ParenthesedSelect) prev,qParser.allConds, qParser.root,true,false,qParser, dbApparameters);
				t.add(temp);
		...
		else if(prev instanceof Select){
			FromListElement temp = null; 
			prev = ((Select)plainSelect.getFromItem()).getPlainSelect().getJoin(0).getFromItem();
			Join aJ = ((Select)plainSelect.getFromItem()).getPlainSelect().getJoin(0);
		...
		else if(plainSelect.getJoins() instanceof Select){
			FromListElement temp = null; 
			prev = ((Select)plainSelect.getFromItem()).getPlainSelect().getJoin(0).getFromItem();
			Join aJ = ((Select)plainSelect.getFromItem()).getPlainSelect().getJoin(0);
			List<Join> jList = new ArrayList<Join>();
		...
		//Application Testing
				...
				*/
				...
				for (int k=0;k<rcList1.size();k++) {
					...
					if (rcList1.get(k).getExpression() instanceof AllColumns ) {
						tempProjectedCols1.addAll(Util.addAllProjectedColumns(qParser.queryAliases,0,qParser));
					}
					else if (rcList1.get(k) instanceof SelectItem){				
						SelectItem rc = (SelectItem)rcList1.get(k);
						...
						if(exp instanceof CaseExpression ) {
							 ...
							 List<WhenClause> whenClauses = ((CaseExpression) exp).getWhenClauses();
...
//					//end
		...
		for(com.xdata.legacy.parsing.QueryParser qp: qParser.getFromClauseSubqueries()){//For From clause subqueries
		...
		for(com.xdata.legacy.parsing.QueryParser qp: qParser.getWhereClauseSubqueries()){//For Where clause subqueries
		...
		*/
		...
		for (int k=0;k<rcList.size();k++) {
			...
			if (rcList.get(k).getExpression() instanceof AllColumns ) {
				tempProjectedCols.addAll(Util.addAllProjectedColumns(qParser.queryAliases,0,qParser));
			}
			else if (rcList.get(k) instanceof SelectItem){				
				SelectItem rc = (SelectItem)rcList.get(k);
				Expression exp = rc.getExpression();	
				String exposedName = "";
				if(exp instanceof Column){
					exposedName = exp.toString();
	 ...
	 */
	@Deprecated
	public static void processResultSetNode (ResultSetNode rsNode,boolean debug,com.xdata.legacy.parsing.QueryParser qParser) throws Exception{
...
private static RelationHierarchyNode generateRelationHierarchyJSQL(FromItem frmItem) throws StandardException{
		...
		if(w instanceof BinaryExpression){
			...
			if(temp instanceof ParenthesedSelect){
				...
				/*	SelectItem resultSet = (SelectItem)ps.getSelectItems().get(0);
					if(resultSet instanceof SelectItem){
						Expression exp1 = ((SelectItem)resultSet).getExpression();
	...
	public static void modifyTreeForCompareSubQ(com.xdata.legacy.parsing.QueryParser qParser) {
		try{
