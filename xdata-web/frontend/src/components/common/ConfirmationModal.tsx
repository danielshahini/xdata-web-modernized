...
const ConfirmationModal: React.FC<ConfirmationModalProps> = ({
  ...
  const colors = {
    ...
    info: 'bg-brand-600 hover:bg-brand-700 shadow-blue-100'
  };
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[100] p-4 animate-fadeIn">
      <div className="bg-white dark:bg-ink-card rounded-2xl p-8 max-w-md w-full shadow-2xl animate-slideUp border border-slate-200 dark:border-ink-border">
        ...
        <div className="flex gap-4">
          <button 
            onClick={onClose}
            className="flex-1 px-6 py-3 rounded-2xl font-bold text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 transition-all"
          >
            {cancelText}
          </button>
          <button 
            onClick={() => { onConfirm(); onClose(); }}
            className={`flex-1 px-6 py-3 rounded-2xl font-bold text-white transition-all shadow-lg dark:shadow-none ${colors[type]}`}
          >
            {confirmText}
          </button>
        </div>
