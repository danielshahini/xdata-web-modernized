...
const CourseMaterials: React.FC = () => {
  ...
  const add = async () => {
    ...
    if (!form.content.trim()) { toast.error(form.type === 'LINK' ? 'Please enter a URL.' : 'Please enter content.'); return; }
    ...
    try {
      ...
      load();
    } catch (e: any) { toast.error('Material could not be added.'); }
  };

  const remove = async (id: number) => {
    try { await api.delete(`/materials/${id}`); load(); toast.success('Removed'); }
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <h2 className="mt-1 section-title text-2xl flex items-center gap-2"><BookMarked size={22} className="text-brand-500" /> Course materials</h2>
      </div>

      <section className="x-card p-6 sm:p-8">
        <h3 className="section-title flex items-center gap-2 mb-5"><Plus size={20} className="text-brand-500" /> New material</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            <label className="x-label">Type</label>
          ...
          <div className="md:col-span-2">
            ...
            <input className="x-input" value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} placeholder="e.g. SQL cheat sheet" />
          </div>
          <div className="md:col-span-2">
            <label className="x-label">{form.type === 'LINK' ? 'URL' : 'Content'}</label>
            {form.type === 'LINK'
        ...
        <div className="mt-5 flex justify-end"><button onClick={add} className="btn-primary"><Plus size={16} /> Add</button></div>
      </section>

      <section className="x-card p-6 sm:p-8">
        <h3 className="section-title mb-5">Existing materials</h3>
        <div className="space-y-3">
          {visible.map(m => (
            <div key={m.id} className="flex items-start justify-between gap-4 p-4 rounded-xl border border-slate-200 dark:border-ink-border bg-slate-50/50 dark:bg-ink-soft/40">
              ...
              <button onClick={() => remove(m.id)} className="icon-btn hover:text-hard shrink-0" aria-label="Remove"><Trash2 size={16} /></button>
            </div>
          ))}
          {visible.length === 0 && <div className="empty-state"><BookMarked className="text-slate-300 dark:text-slate-600 mb-3" size={36} /><p className="text-sm text-slate-500">No material for this course yet.</p></div>}
        </div>
      </section>
    </div>
  );
