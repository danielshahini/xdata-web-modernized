...
const AdminDashboard: React.FC = () => {
  ...
  const loadCourseMembers = async (courseId: number) => {
    ...
    try {
      ...
      toast.error("Members could not be loaded");
    } finally {
  ...
  const handleCreateCourse = async () => {
    ...
    } catch (err) { toast.error('Error creating course'); }
  };
  ...
  return (
    <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-8 animate-fadeIn pb-20">
      <ConfirmationModal 
        ...
        title="Delete course"
        message="Are you sure you want to delete this course? All associated data will be lost."
      />
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 animate-rise">
        <div>
          <p className="kicker">Administration</p>
          <h1 className="mt-1 text-3xl font-display font-extrabold tracking-tight text-slate-900 dark:text-white">Admin Center</h1>
          <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">System-wide management &amp; monitoring.</p>
        </div>
        <div role="tablist" aria-label="View" className="flex gap-1 bg-slate-100 dark:bg-ink-soft p-1 rounded-xl border border-slate-200 dark:border-ink-border">
          {([['users','Users'],['courses','Courses']] as const).map(([id,label]) => (
      ...
      {activeTab === 'courses' && (
        <div className="space-y-6 animate-slideUp">
          <section className="x-card p-6 sm:p-8">
            <h3 className="section-title flex items-center gap-2"><Plus size={20} className="text-brand-500" /> Create new course</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
              <div>
                <label className="x-label">Course name</label>
                <input placeholder="e.g. Databases I" className="x-input"
              ...
              <div>
                ...
                <input placeholder="e.g. DB1-2026" className="x-input font-mono"
                  value={newCourse.instructorCourseId} onChange={e => setNewCourse({...newCourse, instructorCourseId: e.target.value})} />
              </div>
            </div>
            <div className="mt-5 flex justify-end">
              <button onClick={handleCreateCourse} className="btn-primary"><Plus size={16} /> Create course</button>
            </div>
          </section>
            ...
            <table className="w-full">
              <thead>
                <tr>
                  ...
                  <th className="x-th text-right">Action</th>
                </tr>
              </thead>
              <tbody>
                {courses.map(c => (
                    ...
                    <tr onClick={() => toggleCourse(c.id)} className="x-row cursor-pointer">
                      ...
                      <td className="x-td text-right">
                        <button onClick={(e) => { e.stopPropagation(); setDeleteModal({ isOpen: true, courseId: c.id }); }}
                          className="icon-btn hover:text-hard" title="Delete course" aria-label="Delete course">
                          <Trash2 size={18} />
                        </button>
                      </td>
                    </tr>
                      ...
                      <tr className="bg-slate-50/60 dark:bg-ink-soft/30">
                        <td colSpan={3} className="px-6 sm:px-10 py-6">
                          <div className="flex items-center justify-between mb-4">
                            <h4 className="kicker flex items-center gap-2"><Users size={14} /> Course members</h4>
                            <span className="badge-neutral">{courseMembers[c.id]?.length || 0} people</span>
                          </div>
                          {loadingMembers[c.id] ? (
                            <p className="text-sm text-slate-400 animate-pulse">Loading members…</p>
                          ) : courseMembers[c.id]?.length === 0 ? (
                            <p className="text-sm text-slate-500 italic">No members in this course.</p>
                          ) : (
                              ...
                              {courseMembers[c.id]?.map(member => (
                                <div key={member.loginId} className="flex items-center gap-3 p-3 rounded-xl border border-slate-200 dark:border-ink-border bg-white dark:bg-ink-card">
                                  ...
                                  <div className="overflow-hidden">
                                    <p className="text-sm font-semibold text-slate-900 dark:text-white truncate">{member.username || "Unknown"}</p>
                ...
                {courses.length === 0 && (
                  <tr><td className="x-td text-slate-400 italic" colSpan={3}>No courses created yet.</td></tr>
                )}
              </tbody>
            </table>
          </section>
