 	 ...
 	 var txt = "'query " + correctId+" "+boxname+"'";
 	  ...
 	  var newTextAreaDiv = $(document.createElement('div'))
 	     .attr("id", 'TextAreaDiv' + counter);
 	  	...
 	  	var htmlString = 
 	  	"<div style='height: 25px; width:100%;'><label style='float: left;'>SQL answer:</label><input name='newMarks' type='hidden' value='100'></div>"
	    + '<textarea  style="padding:5px;width:98%; height:200px;" class="textForSQL" name="newQuery" id="'+txtBoxId+'"></textarea>'
		+ '<input type="button" class="remove" id="remove" name="" value="Delete"></div>'
...
<br/>
	<div>
		...
		<div class="fieldset">
			<fieldset>
				<legend> Application Details and Instructions</legend>
				<%
					...
					instructions += "<p></p><label><b>Application id: </b></label> <b><label style='color:#657B9E'>" + assignID+"</b></label>";
					instructions += "<p></p><label><b>Application Name: </b></label> <b><label style='color:#657B9E'>" +assignmentName +"</b></label>";
					out.println(instructions);					
					String defaultDataIdsForAssignment = "";
				%>
					</fieldset>			
			<br/>	</div><div class="fieldset">		
			<fieldset>
				...
				try(Connection dbcon = (new DatabaseConnection()).dbConnection()){
						...
						instr +="<input name='maxMarks' type='hidden' value='100'>";
...
// 								 +"</output><div style='float: left; width: 300px; margin-left: 10px;  padding-top: 3px;'><input name='maxMarks' type='range' min='0' max='100' value='100' data-rangeslider-main></div></div></div>";
					...
					try (Connection dbcon = (new DatabaseConnection()).dbConnection()){
								...
								PreparedStatement stmt2 = dbcon
										...
										if(value != null){
						                    ...
						                    if(dsList != null && dsList.length != 0 ){
												...
												instr += "<br/><label style='float:left;'>Please select the default datasets for evaluation.</label><p></p></br>"; 
						...
						%> 
						 ...
						 <script>document.getElementById("optionalschemaid").value = <%=rs.getInt("optionalschemaid")%>;
							...
							<div style="height: 25px; width:100%">
								...
								<input name = 'existingMarks' type='hidden' value='100'>	 	
						...
						instr += "<input name='maxMarks' type='hidden' value='100'>";
						//instr += "</div><div style='float: left; width: 400px'><label style='float:left; margin-left: 10px;'>Marks:</label><output style='float: left; width: 20px;'>"
						//		 +"</output><div style='float: left; width: 300px; margin-left: 10px;  padding-top: 3px;'><input name='maxMarks' type='range' min='0' max='100' value='100' data-rangeslider-main></div></div>";
						out.println(instr); 
						PreparedStatement stmt2 = dbcon
								.prepareStatement("SELECT * from xdata_sampledata where course_id=? and schema_id=?");
								stmt2.setString(1, courseID);
	...
	%>  					<!-- <div>
						...
						<div class="questionelement">
							...
							<p></p><br>						  
								...
								<div style='float: left; width: 400px'>
								<input name = 'newMarks' type='hidden' value="100">
									 	
									
								</div>
								<b></div>
								<textarea name='newQuery' class="textForSQL" id='query <%=qID +" " + newQueryID %>'><%=query %>
								</textarea>
									<input type='hidden' name='newQueries' value='100'>
				...
				<input type="hidden" name="matchAll" value="matchAll">
						...
						<input type="button" onclick="window.location.href = 'ListOfTesterQuestions.jsp?AssignmentID=<%=assignID %>&showQuestions=true'" value="Cancel" name="cancel" id="cancelButton"/>
				</div>
				 </div>
				</div> 
			</fieldset>
