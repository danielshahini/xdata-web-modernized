...
function changeImage(key){
	 var element = document.getElementById("folderIcon"+key);
	  element.setAttribute("src", "images/folderclosed.gif");
}
...
$.fn.jsonbrowser = function(json, userOptions) {
    ...
    function generateHtml($container, json) {
        ...
        for (var key in json) {
        	...
        	{
        		keyMarkup = '<span class="key"><img id="folderIcon' + key + '/"  name="folderIcon' + key+ '/"  src="images/folderopen.gif" border=0 style="padding-top:8px;"></img> ' + key + ':</span>';
        	}
                ...
                if (val === null) { // null
                    ...
                    if (options.collapsible) {
                        ...
                        $li.on('click', '> .key', function() {
                        	/*$(this).parent().children('ul').find($('#
'+key)).attr('src', 'images/folderclosed.gif').load(function(){
