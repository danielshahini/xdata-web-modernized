...
public class preProcessForDataGeneration {
	...
	public void generateDatasetForQuery(int assignmentId, int questionId, int queryId, String course_id)
			throws Exception {
		this.generateDatasetForQuery(assignmentId, questionId, queryId, course_id, 1);
	}

	public void generateDatasetForQuery(int assignmentId, int questionId, int queryId, String course_id, int dummy)
			throws Exception {
		// kumaran
		GenerateCVC1 cvc = new GenerateCVC1();
		String concatenatedQueryId = "A" + assignmentId + "Q" + questionId + "S" + queryId;
		String filePath = "/" + course_id + "/" + concatenatedQueryId;
		...
		try (Connection dbcon = MyConnection.getDatabaseConnection()) {
			...
			DatabaseMetaData mtdt = dbcon.getMetaData();
...
//			
//			String schema_query="select ddltext from xdata_schemainfo where course_id='631' and schema_id='1'";
//			String schema="";
//			try(PreparedStatement stmt=dbcon.prepareStatement(schema_query)){
//				//stmt.setString(1, course_id);
//				//stmt.setInt(2, schema_id);
//				try(ResultSet rs=stmt.executeQuery()){ 
//					rs.next();
//					schema=rs.getString("ddltext");
//
//				}
//			}
//			
//			String sampleData_query="select sample_data from xdata_sampledata where sampledata_id='1'";
//			String sampleData="";
//			try(PreparedStatement stmt=dbcon.prepareStatement(sampleData_query)){
//				//stmt.setInt(1, sampledata_id); or
//				//stmt.setString(1, sample_data_name);
//				try(ResultSet rs=stmt.executeQuery()){ 
//					rs.next();
//					sampleData=rs.getString("sample_data");
//
//				}
//			}
...
//			
			...
			Connection testConn = cvc.getConnection();
			GenerateDataSet genData = new GenerateDataSet();
			cvc.xdatawebFlag = 1;
			Result R = genData.generateDatasetForQuery(testConn, cvc.getQueryId(), cvc.getQueryString(), "", false,
					filePath, obj, 0);
			...
			WriteFileAndUploadDatasets.uploadDataset(cvc, assignmentId, questionId, queryId, course_id, dataSets,
					cvc.getTableMap());
			...
			if (cvc != null && cvc.getConnection() != null) {
				cvc.closeConn();
				logger.log(Level.INFO,
						"\n**************************CONNECTION CLOSED SUCCESSFULLY (preprocessfordatageneration: line 87)*********************************************\n\n");
				System.out.println("CONNECTION CLOSED SUCCESSFULLY (preprocessfordatageneration: line 87)");
			}
		}

	}

	public static void deletePreviousDatasets(GenerateDataset_new g, String query)
				...
				for (int i = 0; i < f2.length; i++) {
					if (f2[i].isDirectory() && f2[i].getName().startsWith("DS")) {
						...
						Utilities.deletePath(
								Configuration.homeDir + "/temp_smt" + g.getFilePath() + "/" + f2[i].getName()); // added
																												// by
																												// ram
					}
				}
		...
		// BufferedWriter ord = new BufferedWriter(new
		// FileWriter(Configuration.homeDir+"/temp_cvc"+g.getFilePath()+"/queries.txt"));
		// BufferedWriter ord1 = new BufferedWriter(new
		// FileWriter(Configuration.homeDir+"/temp_cvc"+g.getFilePath()+"/queries_mutant.txt"));
		BufferedWriter ord = new BufferedWriter(
				new FileWriter(Configuration.homeDir + "/temp_smt" + g.getFilePath() + "/queries.txt")); // added by ram
		BufferedWriter ord1 = new BufferedWriter(
				new FileWriter(Configuration.homeDir + "/temp_smt" + g.getFilePath() + "/queries_mutant.txt")); // added
																												// by
																												// ram
		ord.write(query);
		ord1.write(query);
	...
	public GenerateCVC1 initializeConnectionDetails(GenerateCVC1 cvc) throws Exception {
		...
		// Connection testConn = (new
		// DatabaseConnection().getTesterConnection(cvc.getAssignId())).getTesterConn();
		try {
			logger.log(Level.INFO,
					"\n**************************CONNECTION NOT ABANDONED YET*********************************************\n\n");
			...
			try (Connection conn = MyConnection.getDatabaseConnection()) {
				...
				try (PreparedStatement stmt = conn.prepareStatement(
						"select connection_id, defaultschemaid from xdata_assignment where assignment_id = ?")) {
					stmt.setInt(1, assignmentId);

					try (ResultSet result = stmt.executeQuery()) {
						// Get optional Schema Id for this question
						try (PreparedStatement statement = conn.prepareStatement(
								"select optionalschemaid from xdata_qinfo where assignment_id = ? and question_id= ? ")) {
						...
						if (result.next()) {
							...
							// If optional schema id exists and it is not same as default schema id, then
							// set it as schemaId
				...
				if (connId != 0 && schemaId != 0) {
					...
					if (testConn != null) {
						...
						try (PreparedStatement stmt1 = conn
								.prepareStatement("select ddltext from xdata_schemainfo where schema_id = ?")) {
							...
							try (ResultSet result = stmt1.executeQuery()) {
								...
								if (result.next()) {
									...
									logger.log(Level.INFO,
											"\n**************************initializeConnectionDetails: line 218*********************************************\n\n");

									p.deleteAllTablesFromTestUser(testConn); // added by ram : delete tables if already
																				// exists.
									...
									for (int i = 0; i < inst.length; i++) {
										...
										if (!inst[i].trim().equals("") && !inst[i].trim().contains("drop table")) {
											...
											// String temp = inst[i].replaceAll("(?i)^[ ]*create[ ]+table[ ]+", "create
											// table ##");
											...
											if (dbType.equalsIgnoreCase("MySql")) {
												...
												temp = inst[i].trim().replaceAll("(?i)^\\s*create\\s+table\\s+",
						...
						try (PreparedStatement stmt2 = conn
								.prepareStatement("select sample_data from xdata_sampledata where schema_id = ?")) {
								...
								if (result.next()) {
									...
									for (int i = 0; i < inst.length; i++) {
										...
										if (!inst[i].trim().equals("") && !inst[i].contains("drop table")
												&& !inst[i].contains("delete from")) {
											// Changed for MSSQL TESTING
											// String temp = inst[i].replaceAll("(?i)^[ ]*insert[ ]+into[ ]+", "insert
											// into [xdata].[dbo].##");
											// stmt = assignmentConn.prepareStatement(temp+";");
									...
									long elapsedTime = stopTime - startTime;
									System.out.println(
											"Total time taken for uploading sample data is: " + elapsedTime / 1000F);
									// System.out.print(elapsedTime/1000F+"\n");
								}
							}
						}
