...
public class GetNodeQueryStructure {
	...
	public static Vector<Node> flattenNode(partialMarking.QueryStructure qStructure, Node node) {
			...
			// flattenAllConds()
				...
				if(node.getLhsRhs()!=null){
					...
					if(qStructure.getWhereClauseSubqueries() != null && !qStructure.getWhereClauseSubqueries().isEmpty()){
						qStructure.getWhereClauseSubqueries().get(queryIndex).getAllConds().add(lhsrhs);
					} 
	...
	public static Vector<Vector<Node>> flattenCNF(partialMarking.QueryStructure qStructure, Node node) {
